...
video (unsafeFromSignal -> extAddr) (unsafeFromSignal -> extWrite) =
  ...
  where
    ...
    (fromSignal -> textY, fromSignal -> glyphY) = scale (SNat @FontHeight) . fst . scale (SNat @2) . center $ vgaY
    ...
    rgb = do
        x <- delayI (Nothing @(Index TextWidth)) textX
        y <- delayI (Nothing @(Index TextHeight)) textY
        ...
        pure $ case liftA2 (,) x y of
            ...
            Just (x, y) -> fromBGR $ if pixel `xor` blink then fore else back
